...
class Fourier:
    ...
    def fft_1d_explicit(x):
        ...
        if N <= 16:
            X = Fourier.dft_naive_1d_explicit(x)
        else:
            # split the array into two halves
            X_even = Fourier.fft_1d_explicit(x[::2])
            X_odd = Fourier.fft_1d_explicit(x[1::2])
    ...
    def fft_inverse_1d_explicit(X):
        ...
        # if the array is small enough, use the naive method
        if N <= 16:
            x = Fourier.dft_inverse_1d_explicit(X)
        else:
            # split the array into two halves
            X_even = Fourier.fft_inverse_1d_explicit(X[::2])
            X_odd = Fourier.fft_inverse_1d_explicit(X[1::2])
            # compute the sum of the two halves
            half = N//2
            for k in range(half):
                p = 1/2*X_even[k]
    ...
    def dft_naive_2d(f):
        ...
        for n in range(N):
            F[:,n] = Fourier.dft_naive_1d(f[:,n])
        # use dft_naive_1d to compute the DFT of each row
        for m in range(M):
            F[m,:] = Fourier.dft_naive_1d(F[m,:])
        return F
    ...
    def dft_inverse_2d(F):
        ...
        for n in range(N):
            f[:,n] = Fourier.dft_inverse_1d(F[:,n])
        # use dft_inverse_1d to compute the IDFT of each row
        for m in range(M):
            f[m,:] = Fourier.dft_inverse_1d(f[m,:])
        return f
    ...
    def fft_1d(x):
        ...
        # if the array is small enough, use the naive method
        if N <= 16:
            X = Fourier.dft_naive_1d(x)
        else:
            # split the array into two halves
            X_even = Fourier.fft_1d(x[::2])
            X_odd = Fourier.fft_1d(x[1::2])
            # compute the sum of the two halves
            half = N//2
    ...
    def fft_inverse_1d(X):
        ...
        # if the array is small enough, use the naive method
        if N <= 16:
            x = Fourier.dft_inverse_1d(X)
        else:
            # split the array into two halves
            X_even = Fourier.fft_inverse_1d(X[::2])
            X_odd = Fourier.fft_inverse_1d(X[1::2])
            # compute the sum of the two halves
            half = N//2
            factor = np.exp(2j*np.pi*np.arange(N)/N)
    ...
    def fft_2d(f):
        ...
        for n in range(N):
            F[:,n] = Fourier.fft_1d(f[:,n])
        # use fft_1d to compute the FFT of each row
        for m in range(M):
            F[m,:] = Fourier.fft_1d(F[m,:])
        return F
    ...
    def fft_inverse_2d(F):
        ...
        for n in range(N):
            f[:,n] = Fourier.fft_inverse_1d(F[:,n])
        # use fft_inverse_1d to compute the inverse FFT of each row
        for m in range(M):
            f[m,:] = Fourier.fft_inverse_1d(f[m,:])
        return f